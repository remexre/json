...
use serde_json::{from_reader, from_slice, from_str, from_value, to_string, to_string_pretty,
                 to_value, to_vec, to_writer, Deserializer, Value};
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            let brackets: String = iter::repeat('[')
                .take(1023)
                .chain(iter::repeat(']').take(1023))
                .collect();
            let _: Value = from_str(&brackets).expect("Could not build value");

            let brackets: String = iter::repeat('[').take(1030).collect();
            test_parse_err::<Value>(&[
                (&brackets, "recursion limit exceeded at line 1 column 1024"),
            ]);
        })
